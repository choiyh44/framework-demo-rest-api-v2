...
import kr.co.ensmart.frameworkdemo.common.context.ApplicationContextWrapper;
import kr.co.ensmart.frameworkdemo.common.token.TokenRequest;
import kr.co.ensmart.frameworkdemo.common.token.TokenService;
import kr.co.ensmart.frameworkdemo.common.token.TokenType;
...
public abstract class RestApi {
	...
	public static RestApi client(String url, boolean enableTokenAuth) {
		...
		if (RestApi.TOKEN_SERVICE == null) {
			RestApi.TOKEN_SERVICE = (TokenService)ApplicationContextWrapper.getBean("tokenService");
		}
	...
	private void configAuthorization(HttpHeaders httpHeaders) {
		if ( this.enableTokenAuth ) {
			...
			httpHeaders.setBearerAuth(TOKEN_SERVICE.createToken(
					new TokenRequest().setTokenType(TokenType.SERVICE_TOKEN).setUserName("service").setValidMillis(validMillis)).getToken());
		}
	}
